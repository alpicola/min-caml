...
let pp e =
  ...
  let rec f i e = String.make i ' ' ^ match e with
    ...
    | Neg x -> "Neg" ^ paren x
    | Add (x, y) -> "Add" ^ paren (x ^ ", " ^ y)
    | Sub (x, y) -> "Sub" ^ paren (x ^ ", " ^ y)
    | FNeg x -> "FNeg" ^ paren x
    | FAdd (x, y) -> "FAdd" ^ paren (x ^ ", " ^ y)
    | FSub (x, y) -> "FSub" ^ paren (x ^ ", " ^ y)
    | FMul (x, y) -> "FMul" ^ paren (x ^ ", " ^ y)
    | FDiv (x, y) -> "FDiv" ^ paren (x ^ ", " ^ y)
    | IfEq (x, y, e1, e2) ->
        "IfEq" ^ paren (x ^ ", " ^ y) ^ "\n" ^ f (i + 2) e1 ^ "\n" ^ f (i + 2) e2
    | IfLE (x, y, e1, e2) ->
        "IfLE" ^ paren (x ^ ", " ^ y) ^ "\n" ^ f (i + 2) e1 ^ "\n" ^ f (i + 2) e2
    ...
    | LetRec({ name = n; args = ns; body = e1 }, e2) ->
        ...
        "\n" ^ f (i + 2) e1 ^ "\n" ^ f (i + 2) e2
    | App (x, ys) -> "App" ^ paren (String.concat ", " (x :: ys))
    | Tuple xs -> "Tuple" ^ paren (String.concat ", " xs)
    | LetTuple (ns, x, e) ->
        "LetTuple" ^ paren (paren (String.concat ", " (List.map ppn ns)) ^ ", " ^ x) ^
        "\n" ^ f (i + 2) e
    | Get (x, y) -> "Get" ^ paren (x ^ ", " ^ y)
    | Put (x, y, z) -> "Put" ^ paren (x ^ ", " ^ y ^ ", " ^ z)
    | ExtArray x -> "ExtArray" ^ paren x
    | ExtFunApp (x, ys) -> "ExtFunApp" ^ paren (String.concat ", " (x :: ys))
  in f 0 e
